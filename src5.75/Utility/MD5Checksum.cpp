...
ULONG CMD5Checksum::GetMD5(BYTE* pDestBuf, BYTE* pSrcBuf, UINT nSrcLength)
{
	//entry invariants
	AfxIsValidAddress(pSrcBuf,nSrcLength,FALSE);

	//calculate and return the checksum
	CMD5Checksum MD5Checksum;
	MD5Checksum.Update(pSrcBuf, nSrcLength );

    MD5Checksum.Final(pDestBuf);
	

    return 0;
}

void CMD5Checksum::Final(BYTE* lpBuf)
{
    BYTE Bits[8];
