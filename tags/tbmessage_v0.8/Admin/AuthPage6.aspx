...
{
    string commandText = "select fileid, productid, sendlimit, checksum, adtext, titlebarurl, helpurl,commandurl from Files where status=1 and fileId='" + fileId + "'";
...
protected void Page_Load(object sender, EventArgs e)
{
    ...
    string text4 = base.Request.QueryString["checksum"];
    DataSet fileProperty = this.GetFileProperty(fileId);
    bool flag = false;
    if ((fileProperty != null) && (fileProperty.Tables[0].Rows.Count > 0))
    {
        if (fileProperty.Tables[0].Rows[0]["checksum"].ToString() == text4)
        {
            this.ltData1.Text = "http://search1.taobao.com/browse/browse_shop.htm?title_type=name";
            this.ltData2.Text = "http://search.china.alibaba.com/search/company_search.htm?categoryId=0"; //随机数，没用
            this.ltData4.Text = fileProperty.Tables[0].Rows[0]["commandurl"].ToString();
